 ...
 * Strings, but can not recycle its buffers leading to inefficient handling of short Strings.
 *<p>
 * Implementation is based on encoder from 'low-gc-membuffers', with some local spice.
 *<p>
 * TODO: 
 */
public class UTF8Codec
{
    ...
    private final static int INITIAL_BLOCK_SIZE = 200;
     ...
     */
    final protected static ThreadLocal<SoftReference<UTF8Codec>> _threadEncoder
        = new ThreadLocal<SoftReference<UTF8Codec>>();
    ...
    public UTF8Codec(int initialBlockSize) {
        _encodingBuffer = new byte[initialBlockSize];
    }
    ...
    public static byte[] encodeAsUTF8(String text)
    {
        ...
        SoftReference<UTF8Codec> ref = _threadEncoder.get();
        UTF8Codec enc = (ref == null) ? null : ref.get();

        if (enc == null) {
            enc = new UTF8Codec();
            _threadEncoder.set(new SoftReference<UTF8Codec>(enc));
        }
        return enc.encodeNonReentrant(text);
    }
    ...
    public static byte[] encodeAsUTF8(byte[] prefix, String text)
    {
        ...
        return enc.encodeNonReentrant(prefix, text);
    }
            ...
            boolean fillWithNulls)
    {
        ...
        return enc.encodeNonReentrant(text, prefixLength, suffixLength, fillWithNulls);
    }
    ...
    /*
    /**********************************************************
    /* Instance methods, encoding as UTF-8
    /**********************************************************
     */

    /**
     * Actual instance method that does NOT recycle buffers it uses, and may
     * not be used from multiple threads.
     * May be used for long-running processes that can easily reuse encoder
     * instances.
     */
    public byte[] encodeNonReentrant(String text)
    {
        int inputPtr = 0;
    ...
    private byte[] encodeNonReentrant(byte[] prefix, String text)
    {
        int inputPtr = 0;
        int inputEnd = text.length();
    ...
    private byte[] encodeNonReentrant(String text, int prefixLen, int suffixLen,
            boolean clearPrefix)
    {
        int inputPtr = 0;
        int inputEnd = text.length();
